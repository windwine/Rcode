...
KEY_FILE = Path(__file__).parent.parent / "gptAPI.txt"
CHART_DIR.mkdir(parents=True, exist_ok=True)

MODEL_NAME = "gpt-5.4-mini"
_TEMPERATURE = 0.4
_MAX_TOKENS = 1024
USE_OPENAI_ANALYSIS = os.getenv("USE_OPENAI_ANALYSIS", "1").strip().lower() not in {"0", "false", "no"}
...
def _get_client() -> AsyncOpenAI:
    return AsyncOpenAI(api_key=_read_api_key())


async def call_openai(client: AsyncOpenAI, prompt: str) -> str:
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are a financial analyst specializing in prediction markets. "
                    "Respond ONLY with valid JSON as instructed. No prose outside the JSON."
                ),
            },
...
async def generate_description_openai(client: AsyncOpenAI, ticker: str, market_title: str) -> dict:
...
2. Notable price trends, including both the recent peak and trough (include exact YYYY-MM-DD dates)
3. What the market movement suggests about expectations for this contract

Requirements:
- Base all date references on the data provided — do not assume any fixed calendar event.
- Mention when the most recent significant peak and trough occurred.
...
{{
  "en": "English analysis here (3-4 sentences, use <strong> tags for key numbers and dates)"
    ...
    try:
        raw = await call_openai(client, prompt)
    except Exception as exc:
        print(f"  [Warning] OpenAI request failed for {ticker}: {exc}")
        return {"en": build_fallback_description(stats, market_title)}

    if not raw:
        print(f"  [Warning] Empty response for {ticker}; using fallback")
        return {"en": build_fallback_description(stats, market_title)}
...
def make_chart(ticker: str, title: str, color: str) -> Path:
    ...
    out_path = CHART_DIR / f"{ticker}_openai_en.html"
...
<style>
  ...
  .model-tag {{ display: inline-block; background: rgba(0,120,180,0.15); border: 1px solid rgba(0,160,220,0.3);
               color: #40b4e5; font-size: 13px; padding: 6px 14px; border-radius: 20px; margin-bottom: 24px; }}
...
<div class="desc-container">
  ...
  {cards_html}
    ...
    cmp_path = CHART_DIR / f"{first_ticker}_comparison_openai_en.html"
...
async def main() -> None:
    ...
    if USE_OPENAI_ANALYSIS:
        print(f"\nRunning analysis ({len(markets)} contracts in parallel)...")
        client = _get_client()
        results = await asyncio.gather(*[
            generate_description_openai(client, ticker, title)
            for ticker, title, _ in markets
        ])
    else:
        results = [
            {"en": build_fallback_description(build_stats(ticker), title)}
            for ticker, title, _ in markets
        ]

    contract_descriptions = [
        {"ticker": ticker, "label": title, **result}
        for (ticker, title, _), result in zip(markets, results)
    ]

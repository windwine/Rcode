...
alldataXs = (
    ...
    .with_columns(
    (pl.col("O") / pl.col("C").shift(1) - 1).over("ID").alias("ret")
    )
...
alldataXs = alldataXs.sort(["ID", "Date"])
alldataXs = (
    alldataXs
...
# now ret and mkt_ret are boith in fact the ons_ret
...
df2.write_parquet(f"{pathname}/alldataFeatures_ons.parquet")
...
df3.write_parquet(f"{pathname}/alldataFeatures3_ons.parquet")

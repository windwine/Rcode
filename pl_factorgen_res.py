...
alldata = alldata.sort(["ID", "Date"])
alldata = (
    alldata
    .with_columns((pl.col("ret") - pl.col("market_ret")).alias("res_ret"))
    .with_columns(pl.col("res_ret").alias("ret"))
    .with_columns(((1 + pl.col("ret")).cum_prod().over("ID")).alias("C"))
)

#%% you do not have the H,L based distance now.
# now ret is in fact the res_ret and mkt_ret is still the mkt_ret
...
df2.write_parquet(f"{pathname}/alldataFeatures_res.parquet")
...
df3.write_parquet(f"{pathname}/alldataFeatures3_res.parquet")
